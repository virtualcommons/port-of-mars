...
export class QuizService extends BaseService {
  ...
  async createQuizSubmission(userId: number, quizId: number): Promise<QuizSubmission> {
    const quizSubmission = new QuizSubmission();
    quizSubmission.userId = userId;
    quizSubmission.quizId = quizId;
    return await this.em.getRepository(QuizSubmission).save(quizSubmission);
  }
  ...
  async getIncompleteQuizUsers(): Promise<Array<User>> {
    return await this.em
      .getRepository(User)
      .find({ where: { passedQuiz: false, isVerified: true } });
  }
  ...
  async checkQuizCompletion(userId: number, quizId?: number): Promise<Array<number>> {
    ...
    for (const quizResponse of quizResponses) {
      ...
      logger.debug(
        "Checking response %d for question %d [%d -> %d]",
        quizResponse.id,
        question.id,
        quizResponse.answer,
    ...
    logger.debug(
      "user %d answered the following questions incorrectly: %o",
      userId,
