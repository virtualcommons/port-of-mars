...
const expandCopies = (rges: Array<RawGameEvent>) =>
  _.flatMap(rges, (e: RawGameEvent) => {
    ...
    const gameEvent: Omit<MarsEventData, 'id'> = {
      name: e.name,
      effect: e.effect,
      flavorText: e.flavorText,
      serverActionHandler: e.serverActionHandler,
      clientViewHandler: e.clientViewHandler,
      clientActionHandler: e.clientActionHandler,
      duration: e.duration
    };
...
const marsEvents: Array<MarsEventData> = enumerate(
  expandCopies([
    {
      name: 'Changing Tides',
      copies: 1,
