...
quizRouter.get("/submission/:id", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
quizRouter.post("/submission", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const quizService = getServices().quiz;
    const user = req.user as User;
    if (user) {
    ...
    try {
      ...
      if (isNaN(submissionId)) {
        throw new ServerError({
          code: 400,
          message: `Invalid submission ID ${req.params.submissionId}`,
        });
      }
      ...
      if (isNaN(questionId)) {
        throw new ServerError({
          code: 400,
          message: `Invalid question ID ${req.params.questionId}`,
        });
      }
      // check answer
      ...
      if (isNaN(answer)) {
        throw new ServerError({
          code: 400,
          message: `Invalid quiz response ${req.body.answer}: should have been an integer.`,
        });
      }
...
quizRouter.get("/complete", async (req: Request, res: Response, next: NextFunction) => {
