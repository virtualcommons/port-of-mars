...
describe('trading validations', () => {
  ...
  it('does not send a notification to everyone', () => {
    g.players['Curator'].sendNotification('test');
    g.players['Curator'].sendNotification('notif 2');
    expect(g.players['Curator'].notifications.length).toBe(2);

    g.players['Curator'].deleteNotification(1);
