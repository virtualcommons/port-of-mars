        ...
        <tbody>
            <tr class="data" v-for="resource in resourceNames" :key="resource">
                <td class="resource-name-data">
                    <img class="icon" :src='require(`@port-of-mars/client/assets/icons/${resource}.svg`)'
                    :alt='resource'/>
                    {{resource}}
                </td>
...
export default class InventoryTable extends Vue{
    ...
    resourceCosts(resource: Resource){
        ...
        return this.isVisible ? 
            this.playerData.info.costs[resource] > 1001 ? 'X' : this.playerData.info.costs[resource] : '??';
    }
...
@import '@port-of-mars/client/stylesheets/gamedashboard/global/InventoryTable.scss'
</style>